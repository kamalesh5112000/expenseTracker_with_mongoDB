...
document.getElementById('rzp-button1').onclick=async function buyPremium(e){
    ...
    const res=await axios.get('http://3.88.203.108:5000/purchase/premium',{headers:{"Authorization":token}});
    console.log(res)
    var options={
        "key":res.data.key_id,
        "order_id":res.data.order.id,
        "handler":async function(response){
            await axios.post('http://3.88.203.108:5000/purchase/updatetransaction',{
...
async function submitForm(e){
    ...
    const token = localStorage.getItem('token');
    const res = await axios.post('http://3.88.203.108:5000/addexpense',myobj,{headers:{"Authorization":token}});
    //const res = await axios.post('http://3.88.203.108:5000/expense',myobj)
...
async function display(){
    ...
    const res = await axios.post(`http://3.88.203.108:5000/expense?page=${page}`,{pagelimit:pagelimit},{headers:{"Authorization":token}});
...
async function showLeaderBoardapi(){
    ...
    const token = localStorage.getItem('token');
        const res = await axios.get('http://3.88.203.108:5000/showLeaderBoard',{headers:{"Authorization":token}});
        console.log(res)
...
function delitem(e){
    ...
    iid=li.childNodes[6].textContent;
    ...
    if(e.target.classList.contains('delete')){
        if(confirm("Are you Sure, You want to Delete it?")){
            ...
            const token = localStorage.getItem('token');
            axios.delete(`http://3.88.203.108:5000/expense/${iid}`,{headers:{"Authorization":token}})
            .then(()=>{
                newitem.innerHTML='';
                display();
